export async function submitTrustedReview(base44, user, body) {
  ...
  if (mode === 'app') {
    ...
    const existing = await base44.asServiceRole.entities.Review.filter({ service_request_id: request.id }, '-created_date', 1);
    if (existing.length || request.rating != null) return Response.json({ error: 'Este atendimento já foi avaliado.' }, { status: 409 });
  ...
  await base44.asServiceRole.entities.Review.create({ locksmith_id: locksmith.id, locksmith_name: locksmith.name, reviewer_id: user.id, customer_name: user.full_name || 'Cliente', rating, comment: String(body.comment || '').trim(), work_mode: mode, ...(request ? { service_request_id: request.id, service_type: request.service_type } : {}) });
  ...
  for (let offset = 0; ; offset += 100) {
    const reviews = await base44.asServiceRole.entities.Review.filter({ locksmith_id: locksmith.id }, 'created_date', 100, offset);
