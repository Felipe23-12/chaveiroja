...
export default function CarKeyProgrammingNotice({ programming, hidePriceDetails = false }) {
  ...
  if (programming.dealerOnly) {
    return (
      <div className="p-4 rounded-xl border-2 border-destructive/30 bg-destructive/10 flex gap-2.5">
        <AlertTriangle className="w-5 h-5 text-destructive shrink-0 mt-0.5" />
        <div>
          <p className="text-sm font-medium text-destructive">Somente concessionária</p>
          <p className="text-xs text-destructive mt-0.5">{programming.reason}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 rounded-xl border-2 border-warning/30 bg-warning/10 flex gap-2.5">
      ...
      <div>
        ...
        <p className="text-xs text-warning mt-0.5">{hidePriceDetails ? "Este veículo exige programação online para confeccionar a chave." : programming.reason}</p>
      </div>
    </div>
  );
}