...
export default function MercadoPagoConnectSetup({ onStatusChange }) {
  ...
  return <div className="p-4 rounded-xl border border-border bg-card space-y-4">
    <div className="flex items-start gap-3"><div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center"><Wallet className="w-5 h-5 text-primary" /></div><div><p className="font-semibold">Recebimentos pelo Mercado Pago</p><p className="text-sm text-muted-foreground mt-0.5">Vincule sua conta para receber automaticamente sua parte dos atendimentos.</p></div></div>
    {status?.connected ? <div className="flex items-center gap-2 text-sm text-emerald-700 bg-emerald-50 rounded-lg p-3"><CheckCircle2 className="w-4 h-4" /> Conta Mercado Pago vinculada e pronta para receber.</div> : <div className="flex items-center gap-2 text-sm text-amber-700 bg-amber-50 rounded-lg p-3"><AlertTriangle className="w-4 h-4" /> Vincule a conta que receberá os repasses.</div>}
