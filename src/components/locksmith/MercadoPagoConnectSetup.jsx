...
export default function MercadoPagoConnectSetup({ onStatusChange }) {
  ...
  return <div className="p-4 rounded-xl border border-border bg-card space-y-4">
    ...
    {status?.connected ? <div className="flex items-center gap-2 text-sm text-emerald-700 bg-emerald-50 rounded-lg p-3"><CheckCircle2 className="w-4 h-4" /> Conta Mercado Pago vinculada e pronta para receber.</div> : <div className="flex items-center gap-2 text-sm text-amber-700 bg-amber-50 rounded-lg p-3"><AlertTriangle className="w-4 h-4" /> Sem conexão, o cliente pode pagar: a plataforma recebe e sua parte fica como crédito pendente, sem repasse automático.</div>}
