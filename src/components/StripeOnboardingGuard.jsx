...
export default function StripeOnboardingGuard() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    setPolicy(null);
    setError("");
    base44.functions.invoke("stripeConnect", { action: "onboarding_policy" })
      .then(({ data }) => { if (!disposed) setPolicy(data); })
      .catch((err) => { if (!disposed) setError(err?.response?.data?.error || "Não foi possível verificar seu cadastro de recebimentos."); });
