...
export default function ConductReportCard({ report, users, locksmiths, scores = [], onStatus }) {
  ...
  return <article className="rounded-xl border border-border bg-card p-4 shadow-sm">
    ...
    <div className="mt-4 grid gap-3 md:grid-cols-2"><ReportParticipantCard label="Denunciante" participant={reporter} locksmith={reporterLocksmith} score={reporterScore} fallbackName={report.reporter_name} type={report.reporter_type} /><ReportParticipantCard label="Denunciado" participant={reported} locksmith={reportedLocksmith} score={reportedScore} fallbackName={report.reported_name} type={report.reported_type} /></div>
