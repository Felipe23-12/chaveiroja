...
export default function FeedbackForm() {
  ...
  const submit = async (event) => {
    ...
    try {
      await base44.entities.AppFeedback.create({ category, subject: subject.trim(), message: message.trim() });
  ...
  return <form onSubmit={submit} className="rounded-2xl border border-border bg-card p-5 space-y-5">
    <fieldset disabled={saving} className="space-y-4">
      ...
      <div className="space-y-2"><Label htmlFor="feedback-message">Conte mais</Label><Textarea id="feedback-message" required maxLength={3000} rows={7} value={message} onChange={(e) => setMessage(e.target.value)} placeholder="Descreva sua experiência ou sugestão. Se ocorreu um erro, diga em qual tela e o que você estava tentando fazer." aria-describedby="feedback-hint" /><p id="feedback-hint" className="text-xs text-muted-foreground">Não inclua senhas, CPF ou dados de pagamento. {message.length}/3000 caracteres.</p></div>
    </fieldset>
