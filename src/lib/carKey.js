...
export async function searchFipeAndKeyValue(model, year) {
  ...
  const res = await base44.integrations.Core.InvokeLLM({
    prompt: `Pesquise na Tabela FIPE brasileira o valor médio atual em reais (BRL) do veículo ${input.model} ano ${input.year} (campo fipe_value) e também o valor médio de mercado de uma chave original/codificada desse veículo em concessionárias no Brasil (campo key_value). Retorne apenas números em reais, sem símbolos nem texto.`,
    add_context_from_internet: true,
    model: "gemini_3_flash",
    response_json_schema: {
      type: "object",
